use crate::{
    config::RunnerConfig, identity::StoredIdentity, ENGINE_VERSION, PLUGIN_RUNTIME_VERSION,
...
struct ChallengeRequest<'a> {
    device_public_key_der_base64: &'a str,
    operating_system: &'static str,
    architecture: &'static str,
    application_version: &'static str,
    protocol_version: u16,
    plugin_runtime_version: &'static str,
    capabilities: serde_json::Value,
    safe_folder_labels: Vec<String>,
    browser_engine: Option<serde_json::Value>,
    installed_plugin_versions: Vec<serde_json::Value>,
    tags: &'a [String],
}
...
pub async fn pair(
...
) -> Result<(StoredIdentity, String), String> {
    if token.trim().len() < 24 {
        return Err("Pairing token is invalid.".into());
    }
    ...
    let request = ChallengeRequest {
        ...
        capabilities,
        safe_folder_labels: config
            .allowed_working_directories
            .iter()
            .map(|value| value.display().to_string())
            .collect(),
        browser_engine: None,
        installed_plugin_versions: vec![],
        tags: &config.tags,
    };
    let challenged = client
        .post(format!("{base}/v1/runners/pairing/challenges"))
        .bearer_auth(token)
    ...
    if !challenged.status().is_success() {
        return Err(format!(
            "Pairing challenge failed with status {}.",
            challenged.status()
        ));
    }
    let challenge: ChallengeResponse =
        challenged.json().await.map_err(|error| error.to_string())?;
    if chrono::DateTime::parse_from_rfc3339(&challenge.expires_at)
        .map_err(|_| "Pairing challenge expiry is invalid.")?
        <= Utc::now()
    {
        return Err("Pairing challenge expired.".into());
    }
    let signature = BASE64.encode(key.sign(challenge.challenge.as_bytes()).to_bytes());
    let confirmation = ConfirmationRequest {
        challenge_id: &challenge.challenge_id,
        challenge: &challenge.challenge,
        signature_base64: signature,
        workspace_id: &config.workspace_id,
        display_name: &config.runner_name,
    };
    let confirmed = client
        .post(format!("{base}/v1/runners/pairing/confirm"))
        .bearer_auth(token)
        .header("x-sandbox-request-time", Utc::now().to_rfc3339())
        .json(&confirmation)
        .send()
    ...
    if !confirmed.status().is_success() {
        return Err(format!(
            "Pairing confirmation failed with status {}.",
            confirmed.status()
        ));
    }
    ...
    Ok((
        StoredIdentity {
            runner_id: paired.runner.runner_id.clone(),
            key_id: format!("device-{}", paired.runner.runner_id),
