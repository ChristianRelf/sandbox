...
mod tests {
    ...
    #[tokio::test] async fn condition_follows_false_branch(){let engine=Engine::new(Database::in_memory().unwrap(),Arc::new(LocalHost));let wf=base(vec![node("trigger","manual_trigger",json!({})),node("condition","condition",json!({"left":1,"operator":"greater_than","right":2})),node("yes","set_data",json!({"values":{"path":"yes"}})),node("no","set_data",json!({"values":{"path":"no"}}))],vec![edge("a","trigger","output","condition"),edge("b","condition","true","yes"),edge("c","condition","false","no")]);engine.database().save_workflow(wf.clone()).unwrap();let run=engine.run(wf,json!({}),CancellationToken::new()).await.unwrap();assert_eq!(run.node_executions.iter().find(|n|n.node_id=="yes").unwrap().status,NodeStatus::Skipped);assert_eq!(run.node_executions.iter().find(|n|n.node_id=="no").unwrap().status,NodeStatus::Successful);}
    #[tokio::test] async fn failed_node_skips_dependents_and_keeps_diagnostics(){let engine=Engine::new(Database::in_memory().unwrap(),Arc::new(LocalHost));let mut wf=base(vec![node("trigger","manual_trigger",json!({})),node("http","http_request",json!({"method":"GET","url":"https://not-approved.example","timeoutMs":1000})),node("after","set_data",json!({"values":{"ok":true}}))],vec![edge("a","trigger","output","http"),edge("b","http","output","after")]);wf.settings.permissions.approved_network_domains.clear();engine.database().save_workflow(wf.clone()).unwrap();let run=engine.run(wf,json!({}),CancellationToken::new()).await.unwrap();assert_eq!(run.status,ExecutionStatus::Failed);assert_eq!(run.node_executions.iter().find(|n|n.node_id=="http").unwrap().status,NodeStatus::Failed);let skipped=run.node_executions.iter().find(|n|n.node_id=="after").unwrap();assert_eq!(skipped.status,NodeStatus::Skipped);assert!(skipped.skip_reason.as_deref().unwrap().contains("Dependency"));}
    #[tokio::test] async fn full_manual_http_condition_notification_vertical_slice(){
        let server=MockServer::start().await;Mock::given(method("GET")).and(path("/health")).respond_with(ResponseTemplate::new(200).set_body_json(json!({"service":"up"}))).mount(&server).await;
        let delivered=Arc::new(std::sync::Mutex::new(vec![]));let engine=Engine::new(Database::in_memory().unwrap(),Arc::new(TestHost(delivered.clone())));
        let wf=base(vec![node("trigger","manual_trigger",json!({})),node("http","http_request",json!({"method":"GET","url":format!("{}/health",server.uri()),"timeoutMs":3000})),node("condition","condition",json!({"left":"{{nodes.http.output.status}}","operator":"equals","right":200})),node("notify","desktop_notification",json!({"title":"Healthy","message":"Status {{nodes.http.output.status}}"}))],vec![edge("a","trigger","output","http"),edge("b","http","output","condition"),edge("c","condition","true","notify")]);
        engine.database().save_workflow(wf.clone()).unwrap();let run=engine.run(wf,json!({"type":"manual"}),CancellationToken::new()).await.unwrap();assert_eq!(run.status,ExecutionStatus::Successful);assert_eq!(run.node_executions.len(),4);assert_eq!(delivered.lock().unwrap().as_slice(),["Healthy:Status 200"]);
    }
