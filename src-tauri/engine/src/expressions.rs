...
pub fn inspect_template(source: &str) -> Result<Vec<String>, EngineError> {
    ...
    if openings != closings {
        return Err(expression_error("Expression delimiters are not balanced."));
    }
    for forbidden in [
        "__proto__",
        "prototype",
        ...
        if source.contains(forbidden) {
            return Err(expression_error(
                "Expression contains a forbidden host or prototype operation.",
            ));
        }
    }
    let mut expression_cursor = 0;
    while let Some(relative) = source[expression_cursor..].find("{{") {
        let start = expression_cursor + relative + 2;
        let end = source[start..]
            .find("}}")
            .ok_or_else(|| expression_error("Expression delimiters are not balanced."))?
            + start;
        let body = source[start..end].trim();
        if body.is_empty() || body.contains("..") || body.ends_with('.') {
            return Err(expression_error(
                "Expression contains an incomplete property path.",
            ));
        }
        for alternative in split_top_level(body, "??")? {
            let atom = alternative.trim();
            let head = atom
            ...
            let root_ok = matches!(
                ...
                "input"
                    ...
                    | "object"
                    | "date"
            ) || parse_literal(atom)?.is_some();
            if !root_ok {
                return Err(expression_error(&format!(
                    "Unknown expression root '{head}'."
                )));
            }
        }
    ...
    while let Some(relative) = source[cursor..].find("nodes.") {
        ...
        if end == start {
            return Err(expression_error("nodes requires a node identifier."));
        }
...
pub fn evaluate(source: &str, context: &ExpressionContext<'_>) -> Result<Value, EngineError> {
    ...
    for (index, part) in parts.iter().enumerate() {
        ...
        match result {
            Evaluated::Value(Value::Null) | Evaluated::Missing if index + 1 < parts.len() => {
                continue
            }
            Evaluated::Missing => {
                return Err(expression_error(&format!(
                    "Value '{source}' is missing. Use safe access or ?? to provide a fallback."
                )))
            }
...
fn resolve_string(source: &str, context: &ExpressionContext<'_>) -> Result<Value, EngineError> {
    ...
    while let Some(relative) = source[cursor..].find("{{") {
        ...
        let end = rest
            .find("}}")
            .ok_or_else(|| expression_error("Expression is missing a closing '}}'."))?;
    ...
    if source[cursor..].contains("}}") {
        return Err(expression_error(
            "Expression has an unexpected closing '}}'.",
        ));
    }
...
fn evaluate_path(source: &str, context: &ExpressionContext<'_>) -> Result<Evaluated, EngineError> {
    ...
    let root_value = match root.as_str() {
        ...
        "nodes" => json!(context
            .outputs
            .iter()
            .map(|(id, value)| (id.clone(), json!({"output":value})))
            .collect::<Map<_, _>>()),
    ...
    while cursor < source.len() {
        ...
        let (safe, segment, next) = if remaining.starts_with("?.") {
            ...
            let close = source[offset..]
                .find(']')
                .ok_or_else(|| expression_error("Array access is missing ']'."))?
                + offset;
                ...
                || (raw.starts_with('"') && raw.ends_with('"'))
            {
                ...
                return Err(expression_error(
                    "Bracket access accepts only a numeric index or quoted property.",
                ));
            };
...
fn valid_helper_name(value: &str) -> bool {
    !value.is_empty()
...
fn reject_segment(value: &str) -> Result<(), EngineError> {
    if matches!(value, "__proto__" | "prototype" | "constructor") {
        Err(expression_error(
            "Prototype and constructor traversal is forbidden.",
        ))
    } else {
        Ok(())
    }
...
fn read_identifier(source: &str, start: usize) -> Result<(String, usize), EngineError> {
    ...
    if end == start {
        return Err(expression_error("Expected a property name."));
    }
...
fn split_top_level<'a>(source: &'a str, separator: &str) -> Result<Vec<&'a str>, EngineError> {
    ...
    while index < bytes.len() {
        ...
        if let Some(active) = quote {
            if character == active && (index == 0 || bytes[index - 1] != b'\\') {
                quote = None;
            }
            index += 1;
            continue;
        ...
        match character {
            ...
            ')' | ']' | '}' => {
                depth -= 1;
                if depth < 0 {
                    return Err(expression_error("Expression has unbalanced delimiters."));
                }
            }
            _ => {}
        }
        if depth == 0 && bytes[index..].starts_with(separator) {
    ...
    if quote.is_some() || depth != 0 {
        return Err(expression_error(
            "Expression has an unterminated string or delimiter.",
        ));
    }
    parts.push(&source[start..]);
    Ok(parts)
}
fn split_arguments(source: &str) -> Result<Vec<&str>, EngineError> {
...
fn find_call_open(source: &str) -> Option<usize> {
    source
        .char_indices()
        .find_map(|(index, character)| (character == '(').then_some(index))
}
fn count_openings(value: &str) -> usize {
...
fn expression_error(message: &str) -> EngineError {
    EngineError::Node(format!("Expression error: {message}"))
}
...
mod tests {
    ...
    fn preserves_full_field_types_and_interpolates() {
        let input = json!({"data":{"total":0,"empty":"","nothing":null}});
        let trigger = json!({"body":{"email":"a@example.com"}});
        let outputs = HashMap::from([("extract".into(), json!({"data":{"heading":"Hello"}}))]);
        let workflow = json!({"name":"Example"});
        let execution = json!({"id":"run-1"});
        let env = Map::new();
        let ctx = context(&input, &trigger, &outputs, &workflow, &execution, &env);
        assert_eq!(
            resolve_value(&json!("{{ input.data.total }}"), &ctx).unwrap(),
            json!(0)
        );
        assert_eq!(
            resolve_value(
                &json!("The title is {{ nodes.extract.output.data.heading }}"),
                &ctx
            )
            .unwrap(),
            json!("The title is Hello")
        );
        assert_eq!(
            evaluate("input.data.missing ?? 'fallback'", &ctx).unwrap(),
            json!("fallback")
        );
        assert_eq!(evaluate("input.data?.missing", &ctx).unwrap(), Value::Null);
    }
    #[test]
    fn supports_arrays_and_helpers() {
        let input = json!({"rows":[{"name":"  Ada  "}]});
        let trigger = json!({});
        ...
        let ctx = context(&input, &trigger, &outputs, &workflow, &execution, &env);
        assert_eq!(
            evaluate("string.trim(input.rows[0].name)", &ctx).unwrap(),
            json!("Ada")
        );
        assert_eq!(
            evaluate("array.length(input.rows)", &ctx).unwrap(),
            json!(1)
        );
    }
    #[test]
    fn blocks_host_and_prototype_access() {
        let value = json!({});
        let outputs = HashMap::new();
        let env = Map::new();
        let ctx = context(&value, &value, &outputs, &value, &value, &env);
        assert!(evaluate("process.env.SECRET", &ctx).is_err());
        assert!(evaluate("input.constructor.prototype", &ctx).is_err());
        assert!(evaluate("eval('1')", &ctx).is_err());
    }
    #[test]
    fn static_inspection_rejects_malformed_and_unknown_roots() {
        assert!(inspect_template("{{ trigger..body }}").is_err());
        assert!(inspect_template("{{ process.env }}").is_err());
        assert!(inspect_template("{{ input.value }}").is_ok());
    }
}
