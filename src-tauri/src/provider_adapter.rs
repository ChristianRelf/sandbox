...
impl ProviderOperationAdapter {
    ...
    pub fn poll(
        ...
        let connection = self
            ...
            .map_err(storage)?
            .ok_or_else(|| {
                PluginError::Permission("The selected connection no longer exists.".into())
            })?;
        if connection.provider != provider || connection.status != ConnectionStatus::Connected {
            return Err(PluginError::Permission(
                "The selected polling connection needs attention.".into(),
            ));
        }
        if provider == "github_app" {
            validate_selected_github_repository(&connection, config)?;
        }
        let secret = self.vault.get(credential_id).map_err(PluginError::Host)?;
        let result = match operation {
            "google.calendar.event_changed" => self.poll_google_calendar(
                config,
                cursor,
                &self.google_access_token(credential_id, secret)?,
            ),
            "google.drive.file_changed" => self.poll_google_drive(
                config,
                cursor,
                &self.google_access_token(credential_id, secret)?,
            ),
            "google.sheets.row_added" => self.poll_google_sheets(
                config,
                cursor,
                &self.google_access_token(credential_id, secret)?,
            ),
            "slack.channel_message_posted" => {
                self.poll_slack(config, cursor, access_token(&secret)?)
            }
            "notion.data_source_page_changed" => {
                self.poll_notion(config, cursor, access_token(&secret)?)
            }
            "github.issue_or_pull_request_changed" => {
                self.poll_github_issues(config, cursor, access_token(&secret)?)
            }
            "github.workflow_run_completed" => {
                self.poll_github_runs(config, cursor, access_token(&secret)?)
            }
            _ => Err(PluginError::Permission(format!(
                "Unsupported polling operation '{operation}'."
            ))),
        };
        if let Err(error) = &result {
            self.mark_connection_attention_if_needed(&connection, error);
        }
        result
    }
    ...
    pub fn list_resources(
        ...
        kind: &str,
        parent: Option<&str>,
    ) -> Result<Vec<ProviderResource>, PluginError> {
        let connection = self
            .database
            .get_connection(credential_id)
            .map_err(storage)?
        ...
        if connection.status != ConnectionStatus::Connected
            && !(connection.provider == "github_app" && kind == "github_repository")
        {
            return Err(PluginError::Permission(
                "The selected connection needs attention.".into(),
            ));
        }
        let secret = self.vault.get(credential_id).map_err(PluginError::Host)?;
        let token = if connection.provider == "google_workspace" {
            self.google_access_token(credential_id, secret)?
        } else {
        ...
        match (connection.provider.as_str(), kind) {
            ("github_app", "github_repository") => {
                ...
                for installation in installations {
                    ...
                    let result=response_json(self.github_request(Method::GET,format!("https://api.github.com/user/installations/{id}/repositories?per_page=100"),&token).send().map_err(host)?,"List GitHub installation repositories")?;
                    for repository in result
                        ...
                        .unwrap_or_default()
                    {
                        if let (Some(full_name), Some(repo_id)) = (
                            repository.get("full_name").and_then(Value::as_str),
                            repository.get("id"),
                        ) {
                            output.push(ProviderResource{id:full_name.into(),label:full_name.into(),metadata:json!({"repositoryId":repo_id,"installationId":id,"owner":repository.pointer("/owner/login"),"private":repository.get("private"),"permissions":repository.get("permissions")})});
                        }
                    }
                }
                Ok(output)
            }
            ("github_app", "github_workflow") => {
                let repository =
                    parent.ok_or_else(|| PluginError::Host("Select a repository first.".into()))?;
                validate_repository(repository)?;
                let result=response_json(self.github_request(Method::GET,format!("https://api.github.com/repos/{repository}/actions/workflows?per_page=100"),&token).send().map_err(host)?,"List GitHub workflows")?;
                Ok(result.get("workflows").and_then(Value::as_array).cloned().unwrap_or_default().into_iter().filter_map(|item|Some(ProviderResource{id:item.get("path")?.as_str()?.rsplit('/').next()?.into(),label:item.get("name")?.as_str()?.into(),metadata:json!({"workflowId":item.get("id"),"path":item.get("path"),"state":item.get("state")})})).collect())
            }
            ("github_app", "github_branch") => {
                let repository =
                    parent.ok_or_else(|| PluginError::Host("Select a repository first.".into()))?;
                validate_repository(repository)?;
                ...
                Ok(result.as_array().cloned().unwrap_or_default().into_iter().filter_map(|item|Some(ProviderResource{id:item.get("name")?.as_str()?.into(),label:item.get("name")?.as_str()?.into(),metadata:json!({"headSha":item.pointer("/commit/sha"),"protected":item.get("protected")})})).collect())
            }
            ("slack_oauth", "slack_channel") => {
                let result = self.slack_json(
                    "conversations.list",
                    &token,
                    Some(&[
                ...
                Ok(result.get("channels").and_then(Value::as_array).cloned().unwrap_or_default().into_iter().filter_map(|item|Some(ProviderResource{id:item.get("id")?.as_str()?.into(),label:format!("#{}",item.get("name")?.as_str()?),metadata:json!({"private":item.get("is_private"),"archived":item.get("is_archived")})})).collect())
            }
            ("notion", "notion_data_source") => {
                let result=response_json(self.client.post("https://api.notion.com/v1/search").bearer_auth(&token).header("Notion-Version",NOTION_API_VERSION).json(&json!({"filter":{"property":"object","value":"data_source"},"page_size":100})).send().map_err(host)?,"List Notion data sources")?;
                Ok(result
                    .get("results")
                    .and_then(Value::as_array)
                    .cloned()
                    ...
                    .into_iter()
                    .filter_map(|item| {
                        Some(ProviderResource {
                            id: item.get("id")?.as_str()?.into(),
                            label: notion_title(&item),
                            metadata: json!({"url":item.get("url"),"parent":item.get("parent")}),
                        })
                    })
                    .collect())
            }
            ("google_workspace", "google_calendar") => {
                ...
                Ok(result.get("items").and_then(Value::as_array).cloned().unwrap_or_default().into_iter().filter_map(|item|Some(ProviderResource{id:item.get("id")?.as_str()?.into(),label:item.get("summary").and_then(Value::as_str).unwrap_or("Calendar").into(),metadata:json!({"primary":item.get("primary"),"accessRole":item.get("accessRole")})})).collect())
            }
            ("google_workspace", "google_spreadsheet") => {
                let result=response_json(self.client.get("https://www.googleapis.com/drive/v3/files").bearer_auth(&token).query(&[("q","mimeType='application/vnd.google-apps.spreadsheet' and trashed=false"),("pageSize","1000"),("fields","files(id,name,modifiedTime,webViewLink)")]).send().map_err(host)?,"List Google spreadsheets")?;
                Ok(result.get("files").and_then(Value::as_array).cloned().unwrap_or_default().into_iter().filter_map(|item|Some(ProviderResource{id:item.get("id")?.as_str()?.into(),label:item.get("name")?.as_str()?.into(),metadata:json!({"modifiedTime":item.get("modifiedTime"),"webViewLink":item.get("webViewLink")})})).collect())
            }
            _ => Err(PluginError::Permission(format!(
                "Resource picker '{kind}' is not available for {}.",
                connection.provider
            ))),
        }
    }

    fn mark_connection_attention_if_needed(
        &self,
        ...
        if connection.provider != "github_app"
            || (!message.contains("permission_missing")
                && !message.contains("resource_not_found")
                && !message.contains("authentication_expired"))
        {
            return;
        }
        let mut updated = connection.clone();
        updated.status = ConnectionStatus::Error;
        updated.metadata["attentionReason"] = Value::String(message.chars().take(1000).collect());
        updated.metadata["lastValidationFailureAt"] = json!(Utc::now());
        let _ = self.database.save_connection(&updated);
    }

    fn poll_google_calendar(
        &self,
        config: &Value,
        cursor: Option<&Value>,
        token: &str,
        ...
        if let Some(sync) = cursor
            .and_then(|value| value.get("syncToken"))
            .and_then(Value::as_str)
        {
            request = request.query(&[("syncToken", sync)]);
        } else {
            request = request.query(&[("timeMin", Utc::now().to_rfc3339())]);
        }
        ...
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"syncToken":result.get("nextSyncToken"),"observedAt":Utc::now()}),
        })
    }

    fn poll_google_drive(
        &self,
        config: &Value,
        cursor: Option<&Value>,
        token: &str,
        ...
        let page_token = cursor
            .and_then(|value| value.get("pageToken"))
            .and_then(Value::as_str);
        if page_token.is_none() {
            let start = response_json(
                self.client
                    ...
                    .send()
                    .map_err(host)?,
                "Drive polling baseline",
            )?;
            return Ok(PollBatch {
                events: vec![],
                event_keys: vec![],
                cursor: json!({"pageToken":start.get("startPageToken")}),
            });
        }
        ...
        if let Some(drive) = optional_str(config, "driveId") {
            request = request.query(&[("driveId", drive), ("corpora", "drive")]);
        }
        let result = response_json(request.send().map_err(host)?, "Drive polling")?;
        let events = result
            .get("changes")
            .and_then(Value::as_array)
            .cloned()
        ...
        let next = result
            .get("nextPageToken")
            .or_else(|| result.get("newStartPageToken"))
            .cloned()
            .unwrap_or_else(|| Value::String(page_token.unwrap().into()));
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"pageToken":next}),
        })
    }

    fn poll_google_sheets(
        &self,
        config: &Value,
        cursor: Option<&Value>,
        token: &str,
    ) -> Result<PollBatch, PluginError> {
        let sheet = required_str(config, "spreadsheetId", "Spreadsheet is required.")?;
        let range = required_str(config, "range", "Range is required.")?;
        let result = response_json(
            self.client
                .get(format!(
        ...
        let rows = result
            ...
            .cloned()
            .unwrap_or_default();
        let header = config.get("headerRow").and_then(Value::as_u64).unwrap_or(1) as usize;
        let previous = cursor
            .and_then(|value| value.get("lastRow"))
            .and_then(Value::as_u64)
            .unwrap_or(rows.len() as u64) as usize;
        let events = rows
            .iter()
            .enumerate()
            .skip(previous.max(header))
            .map(|(index, row)| json!({"rowNumber":index+1,"values":row}))
            .collect::<Vec<_>>();
        let keys = events
            .iter()
            .filter_map(|event| {
                event
                    .get("rowNumber")
                    .map(|row| format!("{sheet}:{range}:{row}"))
            })
            .collect();
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"lastRow":rows.len()}),
        })
    }
    ...
    fn poll_slack(
        ...
        if let Some(oldest) = cursor
            .and_then(|value| value.get("latestTs"))
            .and_then(Value::as_str)
        {
            query.push(("oldest", oldest.into()));
            query.push(("inclusive", "false".into()));
        }
        let result = self.slack_json("conversations.history", token, Some(&query), None)?;
        let include_bots = config
            .get("includeBotMessages")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        let include_replies = config
            .get("includeThreadReplies")
            .and_then(Value::as_bool)
            .unwrap_or(false);
        let mut events = result
            ...
            .into_iter()
            .filter(|message| {
                (include_bots || message.get("bot_id").is_none())
                    && (include_replies
        ...
        let latest = events
            .iter()
            .filter_map(|value| value.get("ts").and_then(Value::as_str))
            .max()
            .or_else(|| {
                cursor
                    .and_then(|value| value.get("latestTs"))
    ...
    fn poll_notion(
        ...
        let previous = cursor
            .and_then(|value| value.get("lastEditedTime"))
            .and_then(Value::as_str)
            .unwrap_or("");
        let mut pages = result
            ...
            .filter(|page| {
                page.get("last_edited_time")
                    .and_then(Value::as_str)
                    .is_some_and(|time| time > previous)
            })
            .collect::<Vec<_>>();
        let latest = pages
            .iter()
            .filter_map(|page| page.get("last_edited_time").and_then(Value::as_str))
            .max()
            .unwrap_or(previous)
            .to_string();
        let keys = pages
            ...
            .filter_map(|page| {
                Some(format!(
                    "{}:{}",
                    page.get("id")?.as_str()?,
                    page.get("last_edited_time")?.as_str()?
                ))
            })
            .collect();
        pages.reverse();
        ...
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"lastEditedTime":if latest.is_empty(){Utc::now().to_rfc3339()}else{latest}}),
        })
    }

    fn poll_github_issues(
        &self,
        config: &Value,
        cursor: Option<&Value>,
        token: &str,
    ) -> Result<PollBatch, PluginError> {
        let repository = required_str(config, "repository", "Repository is required.")?;
        validate_repository(repository)?;
        let repository_metadata = response_json(
            self.github_request(
                Method::GET,
        ...
        let repository_id = repository_metadata
            .get("id")
        ...
        let mut query = vec![
            (
                "state",
                optional_str(config, "state").unwrap_or("all").to_string(),
            ),
            ("per_page", "100".into()),
            ("sort", "updated".into()),
            ("direction", "asc".into()),
        ...
        let record_type = optional_str(config, "recordType").unwrap_or("both");
        let labels = config
            .get("labels")
            .and_then(Value::as_array)
            .cloned()
        ...
        let latest = provider_records
            .iter()
            .filter_map(|item| item.get("updated_at").and_then(Value::as_str))
            .max()
            .or(previous)
            .unwrap_or("")
            .to_string();
        let records = provider_records
            .into_iter()
            .filter(|item| {
                let is_pr = item.get("pull_request").is_some();
                (record_type == "both"
                    || (record_type == "pull_request" && is_pr)
                    || (record_type == "issue" && !is_pr))
                    && (labels.is_empty()
                        || item
                            .get("labels")
                            .and_then(Value::as_array)
                            .is_some_and(|current| {
                                labels.iter().all(|wanted| {
                                    current
                                        .iter()
                                        .any(|label| label.get("name") == Some(wanted))
                                })
                            }))
                    && (actors.is_empty()
                        || actors
                            .iter()
                            .any(|actor| item.pointer("/user/login") == Some(actor)))
            })
            .collect::<Vec<_>>();
        let keys = records
            .iter()
        ...
        for item in records {
            let pull = if item.get("pull_request").is_some() {
                let number = item.get("number").and_then(Value::as_u64).ok_or_else(|| {
                    PluginError::Host("GitHub pull request had no number.".into())
                })?;
                Some(response_json(
                    self.github_request(
                        Method::GET,
                    ...
                    .send()
                    .map_err(host)?,
                    "Enrich polled GitHub pull request",
                )?)
            } else {
                None
            };
            let kind = if previous.is_some_and(|since| {
                item.get("created_at")
                    .and_then(Value::as_str)
        ...
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"updatedAt":if latest.is_empty(){Utc::now().to_rfc3339()}else{latest}}),
        })
    }
    ...
    fn poll_github_runs(
        ...
        for (target, source) in [("branch", "branch"), ("event", "event")] {
            if let Some(value) = optional_str(config, source) {
                query.push((target, value.into()));
            }
        }
        let workflow = optional_str(config, "workflow");
        let url = workflow
            .map(|value| {
                format!(
                    "https://api.github.com/repos/{repository}/actions/workflows/{}/runs",
                    urlencoding::encode(value)
                )
            })
            .unwrap_or_else(|| format!("https://api.github.com/repos/{repository}/actions/runs"));
        let result = response_json(
            self.github_request(Method::GET, url, token)
                .query(&query)
                .send()
                .map_err(host)?,
            "GitHub workflow polling",
        )?;
        let previous = cursor
            ...
            .unwrap_or("");
        let workflow = optional_str(config, "workflow");
        let conclusions = config
            .get("conclusions")
            .and_then(Value::as_array)
            .cloned()
            .unwrap_or_default();
        let allow_all = conclusions.is_empty()
            || conclusions
                .iter()
        ...
        let provider_runs = result
            ...
            .cloned()
            .unwrap_or_default();
        let latest = provider_runs
            .iter()
            .filter_map(workflow_completed_time)
            .max()
            .unwrap_or(previous)
            .to_string();
        let mut events = provider_runs
            .into_iter()
            .filter(|run| {
                workflow_completed_time(run).is_some_and(|time| time > previous)
                    && workflow.is_none_or(|wanted| {
                        run.get("workflow_id")
                            .map(|value| value.to_string())
                            .as_deref()
                            == Some(wanted)
                            || run.get("name").and_then(Value::as_str) == Some(wanted)
                            || run.get("path").and_then(Value::as_str).is_some_and(|path| {
                    ...
                    && (allow_all
                        || conclusions
                            .iter()
                            .any(|value| run.get("conclusion") == Some(value)))
            })
            .collect::<Vec<_>>();
        ...
        Ok(PollBatch {
            events,
            event_keys: keys,
            cursor: json!({"completedAt":if latest.is_empty(){Utc::now().to_rfc3339()}else{latest}}),
        })
    }
    ...
    fn execute_inner(
        ...
        if provider == "github_app" {
            validate_selected_github_repository(&connection, configuration)?;
        }
        let secret = self.vault.get(credential_id).map_err(PluginError::Host)?;
    ...
    fn google_access_token(
        ...
        let mut connection = self
            ...
            .map_err(storage)?
            .ok_or_else(|| {
                PluginError::Permission("Google Workspace connection is missing.".into())
            })?;
        ...
        if needs_refresh {
            let refresh = secret
                ...
                .and_then(Value::as_str)
                .ok_or_else(|| {
                    PluginError::Permission("Google authorization expired; reconnect it.".into())
                })?;
            let client_id = std::env::var("SANDBOX_GOOGLE_WORKSPACE_CLIENT_ID")
                .or_else(|_| std::env::var("SANDBOX_GMAIL_CLIENT_ID"))
                .map_err(|_| {
                    PluginError::Host("Google Workspace OAuth client is not configured.".into())
                })?;
            let response = self
                .client
        ...
        match operation {
            ...
            "google.calendar.create_event" | "google.calendar.update_event" => {
                ...
                let request = if operation.ends_with("update_event") {
                    let event =
                        required_str(config, "eventId", "Update Calendar Event requires eventId.")?;
                ...
                response_json(
                    request
                        ...
                        .send()
                        .map_err(host)?,
                    if operation.ends_with("update_event") {
            ...
            "google.drive.upload_file" => {
                let grant = required_str(
                    config,
                    "fileGrant",
                    "Upload Drive File requires a file grant.",
                )?;
                let (path, bytes) = self.file_bytes(grant, input)?;
                let name = optional_str(config, "name")
                    .or_else(|| {
                        Path::new(&path)
                            .file_name()
                ...
                if let Some(parent) = optional_str(config, "parentFolderId") {
            ...
            "google.sheets.read_range" => {
                let sheet = required_str(
                    config,
                    "spreadsheetId",
                    "Read Sheet Range requires spreadsheetId.",
                )?;
                ...
                response_json(
                    self.client
                        ...
                        .query(&[(
                            "majorDimension",
                            optional_str(config, "majorDimension").unwrap_or("ROWS"),
                        )])
                        .send()
                        .map_err(host)?,
                    "Read Sheet Range",
            ...
            "google.sheets.append_rows" | "google.sheets.update_range" => {
                let sheet = required_str(
                    config,
                    "spreadsheetId",
                    "Sheets action requires spreadsheetId.",
                )?;
                let range = required_str(config, "range", "Sheets action requires range.")?;
                let values = config
                    .get(if operation.ends_with("append_rows") {
                        "rows"
                    } else {
                        "values"
                    })
                ...
                let request = if operation.ends_with("append_rows") {
                    self.client.post(format!("{base}:append")).query(&[
                        ("valueInputOption", option),
                        ("insertDataOption", "INSERT_ROWS"),
                    ])
                } else {
                    self.client.put(base).query(&[("valueInputOption", option)])
                };
                response_json(
                    request
                        .bearer_auth(token)
            ...
            _ => Err(PluginError::Permission(format!(
                "Unsupported Google operation '{operation}'."
            ))),
        }
    }
    ...
    fn slack_json(
        ...
        let mut request = if body.is_some() {
            self.client.post(format!("https://slack.com/api/{method}"))
        } else {
            self.client.get(format!("https://slack.com/api/{method}"))
        }
        .bearer_auth(token);
        ...
        let (method, url, body) = match operation {
            "notion.query_data_source" => (
                ...
                format!(
                    ...
                    required_str(config, "dataSourceId", "Data source is required.")?
                ),
                json!({"filter":config.get("filter"),"sorts":config.get("sorts"),"page_size":config.get("pageSize").and_then(Value::as_u64).unwrap_or(100)}),
            ),
            "notion.get_page" => (
                Method::GET,
                format!(
                    "https://api.notion.com/v1/pages/{}",
                    required_str(config, "pageId", "Page ID is required.")?
                ),
                Value::Null,
            ),
            "notion.create_page" => (
                ...
                json!({"parent":{optional_str(config,"parentType").unwrap_or("data_source_id"):required_str(config,"parentId","Parent ID is required.")?},"properties":config.get("properties").cloned().unwrap_or_else(||json!({})),"children":config.get("children").cloned().unwrap_or_else(||json!([]))}),
            ),
            "notion.update_page" => (
                Method::PATCH,
                format!(
                ...
                json!({"properties":config.get("properties").cloned().unwrap_or_else(||json!({})),"archived":config.get("archived").cloned().unwrap_or(Value::Null)}),
            ),
            _ => {
                return Err(PluginError::Permission(format!(
                    "Unsupported Notion operation '{operation}'."
                )))
            }
        };
        let mut request = self
    ...
    fn github(&self, operation: &str, config: &Value, token: &str) -> Result<Value, PluginError> {
        ...
        if operation == "github.get_issue_or_pull_request" {
            let issue = response_json(
                self.github_request(
                    Method::GET,
                    format!("{base}/issues/{}", number(config, "number")?),
                    token,
                )
                .send()
                .map_err(host)?,
                "Get GitHub issue or pull request",
            )?;
            let pull = if issue.get("pull_request").is_some() {
                Some(response_json(
                    self.github_request(
                        Method::GET,
                        format!("{base}/pulls/{}", number(config, "number")?),
                        token,
                    )
                    .send()
                    .map_err(host)?,
                    "Enrich GitHub pull request",
                )?)
            } else {
                None
            };
        ...
        let (method, url, body, empty_ok) = match operation {
            ...
            "github.add_comment" => (
                Method::POST,
                format!("{base}/issues/{}/comments", number(config, "number")?),
                json!({"body":required_str(config,"body","Comment body is required.")?}),
                false,
            ),
            ...
            "github.request_reviewers" => {
                ...
                let teams = config
                    ...
                    .and_then(Value::as_array)
                    .map_or(0, Vec::len);
                if reviewers + teams == 0 {
                    return Err(PluginError::Host(
                        "Request Reviewers needs at least one user or team.".into(),
                    ));
                }
                (
                    Method::POST,
                    format!(
                        "{base}/pulls/{}/requested_reviewers",
                        number(config, "number")?
                    ),
                    json!({"reviewers":config.get("reviewers").cloned().unwrap_or_else(||json!([])),"team_reviewers":config.get("teamReviewers").cloned().unwrap_or_else(||json!([]))}),
                    false,
                )
            }
            "github.merge_pull_request" => (
                Method::PUT,
                format!("{base}/pulls/{}/merge", number(config, "number")?),
                compact_json(
                    config,
                    &[
                        ("sha", "expectedHeadSha"),
            ...
            "github.get_workflow_run" => (
                Method::GET,
                format!("{base}/actions/runs/{}", number(config, "runId")?),
                Value::Null,
                false,
            ),
            "github.dispatch_workflow" => (
                Method::POST,
                format!(
                    "{base}/actions/workflows/{}/dispatches",
                    urlencoding::encode(required_str(config, "workflow", "Workflow is required.")?)
                ),
                json!({"ref":required_str(config,"ref","Git ref is required.")?,"inputs":config.get("inputs").cloned().unwrap_or_else(||json!({}))}),
                true,
            ),
            "github.cancel_workflow_run" => (
                Method::POST,
                format!("{base}/actions/runs/{}/cancel", number(config, "runId")?),
                Value::Null,
                true,
            ),
            _ => {
        ...
        if operation == "github.cancel_workflow_run" && response.status() == StatusCode::CONFLICT {
            return Ok(
                json!({"accepted":false,"runId":config.get("runId"),"reason":"already_completed","requestedAt":Utc::now()}),
            );
        }
        if empty_ok && response.status().is_success() {
            return Ok(if operation == "github.dispatch_workflow" {
                json!({"accepted":true,"repository":repository,"workflow":config.get("workflow"),"ref":config.get("ref"),"dispatchedAt":Utc::now()})
            } else {
                json!({"accepted":true,"runId":config.get("runId"),"requestedAt":Utc::now()})
            });
        }
        let result = response_json(response, "GitHub API")?;
        Ok(match operation {
            "github.create_issue" | "github.update_issue" => {
                normalize_issue_or_pull(repository, &result, None)
            }
            "github.add_comment" => normalize_comment(&result),
            "github.create_pull_request" | "github.update_pull_request" => {
                normalize_pull(repository, &result)
            }
            "github.request_reviewers" => {
                json!({"requestedUsers":result.get("requested_reviewers").cloned().unwrap_or_else(||json!([])),"requestedTeams":result.get("requested_teams").cloned().unwrap_or_else(||json!([]))})
            }
            "github.merge_pull_request" => {
                json!({"merged":result.get("merged").cloned().unwrap_or(Value::Bool(false)),"mergeSha":result.get("sha"),"message":result.get("message"),"url":format!("https://github.com/{repository}/pull/{}",number(config,"number")?)})
            }
    ...
    fn file_bytes(&self, grant: &str, input: &Value) -> Result<(String, Vec<u8>), PluginError> {
        if !input
            .get("fileGrants")
            .and_then(Value::as_array)
            .is_some_and(|values| values.iter().any(|value| value.as_str() == Some(grant)))
        {
            return Err(PluginError::Permission(
                "The file was not granted to this invocation.".into(),
            ));
        }
        let (path, maximum) = self
            .database
            .resolve_file_grant(grant)
            .map_err(storage)?
            .ok_or_else(|| {
                PluginError::Permission(
                    "The file grant is missing, expired, or already consumed.".into(),
                )
            })?;
        let metadata = fs::metadata(&path).map_err(host)?;
        if !metadata.is_file() {
            return Err(PluginError::Permission(
                "File grants cannot reference directories.".into(),
            ));
        }
        if metadata.len() > maximum {
            return Err(PluginError::ResourceLimit(format!(
                "File exceeds the {maximum}-byte grant limit."
            )));
        }
...
fn response_json(
    ...
    if !status.is_success() {
        return Err(provider_status(action, response));
    }
    response
        .json()
        .map_err(|error| PluginError::Host(format!("{action} returned invalid JSON: {error}")))
}
...
fn provider_status(action: &str, response: reqwest::blocking::Response) -> PluginError {
    ...
    let rate_remaining_zero = response
        ...
        .and_then(|value| value.to_str().ok())
        == Some("0");
    let reset_delay = response
        .headers()
        .get("x-ratelimit-reset")
        .and_then(|value| value.to_str().ok())
        .and_then(|value| value.parse::<i64>().ok())
        .map(|reset| (reset - Utc::now().timestamp()).max(1).to_string());
    ...
    let classification = if rate_limited {
        "rate_limited"
    } else {
        match status.as_u16() {
            ...
            409 => "conflict_or_stale_revision",
            422 => "validation_failure",
            500..=599 => "transient_provider_failure",
            _ => "permanent_provider_failure",
        }
    };
    PluginError::Host(format!(
        "{classification}: {action} failed with HTTP {status}{}: {detail}",
        retry
...
fn calendar_body(config: &Value) -> Value {
    ...
    for key in ["start", "end"] {
        if let Some(value) = optional_str(config, key) {
            body[key] = json!({"dateTime":value,"timeZone":optional_str(config,"timeZone")});
        }
    }
    if let Some(attendees) = body.get_mut("attendees").and_then(Value::as_array_mut) {
        *attendees = attendees
            .iter()
...
fn access_token(secret: &Value) -> Result<&str, PluginError> {
    required_str(
        secret,
        "accessToken",
        "The connection access token is unavailable; reconnect it.",
    )
}
...
fn required_str<'a>(value: &'a Value, key: &str, message: &str) -> Result<&'a str, PluginError> {
    optional_str(value, key).ok_or_else(|| PluginError::Host(message.into()))
}
fn number(value: &Value, key: &str) -> Result<u64, PluginError> {
    value
        .get(key)
        .and_then(Value::as_u64)
        .ok_or_else(|| PluginError::Host(format!("{key} must be a positive integer.")))
}
fn push_query(
...
fn validate_repository(value: &str) -> Result<(), PluginError> {
        ...
        })
    {
        Err(PluginError::Host(
            "Repository must use owner/name format.".into(),
        ))
    } else {
        Ok(())
    }
}
fn workflow_completed_time(run: &Value) -> Option<&str> {
    run.get("completed_at")
        .or_else(|| run.get("updated_at"))
        .and_then(Value::as_str)
}
fn validate_selected_github_repository(
    connection: &ConnectionMetadata,
    config: &Value,
) -> Result<(), PluginError> {
    let Some(repository) = optional_str(config, "repository") else {
        return Ok(());
    };
    ...
    if !selected.iter().any(|item| {
        item.as_str() == Some(repository)
            || item.get("fullName").and_then(Value::as_str) == Some(repository)
    }) {
        return Err(PluginError::Permission(format!(
            "Repository '{repository}' is not selected for this GitHub App connection."
        )));
    }
    Ok(())
}
fn notion_title(value: &Value) -> String {
    value
        .get("title")
        .and_then(Value::as_array)
        .and_then(|parts| {
        ...
        .unwrap_or("Untitled data source")
        .to_string()
}
...
fn normalize_comment(comment: &Value) -> Value {
    json!({"commentId":comment.get("id"),"nodeId":comment.get("node_id"),"body":comment.get("body"),"author":comment.pointer("/user/login"),"createdAt":comment.get("created_at"),"updatedAt":comment.get("updated_at"),"url":comment.get("html_url")})
}
fn normalize_workflow_run(run: &Value) -> Value {
    json!({"runId":run.get("id"),"runAttempt":run.get("run_attempt"),"workflowId":run.get("workflow_id"),"workflowName":run.get("name"),"status":run.get("status"),"conclusion":run.get("conclusion"),"event":run.get("event"),"branch":run.get("head_branch"),"headSha":run.get("head_sha"),"actor":run.pointer("/actor/login"),"createdAt":run.get("created_at"),"updatedAt":run.get("updated_at"),"runStartedAt":run.get("run_started_at"),"completedAt":run.get("updated_at"),"htmlUrl":run.get("html_url"),"jobsUrl":run.get("jobs_url"),"artifactsUrl":run.get("artifacts_url")})
}
...
fn host(error: impl std::fmt::Display) -> PluginError {
    PluginError::Host(error.to_string())
}
