...
function repository(permissionByWorkspace: Record<string, Permission[]>): ControlPlaneRepository {
  return {
    ...
    authenticateRunnerRequest: vi.fn(), recordRunnerHeartbeat: vi.fn(), dequeueRunnerCommands: vi.fn(), updateRunnerCommandStatus: vi.fn(), recordRunSummary: vi.fn(), listWorkspaceActivity: vi.fn(), listDeployments:vi.fn(), createDeployment:vi.fn(), transitionDeployment:vi.fn(), listRunnerPools:vi.fn(), createRunnerPool:vi.fn(), updateRunnerPool:vi.fn(), deleteRunnerPool:vi.fn(),
