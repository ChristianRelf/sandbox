...
function RecordingReview({ steps, setSteps, onCancel, onApply }: { steps: RecordedStep[]; setSteps: (steps: RecordedStep[]) => void; onCancel: () => void; onApply: () => void }) {
  ...
  return <div className="overlay"><div className="recording-review">
    ...
    <section>{steps.length ? steps.map((step, index) => <div className="recorded-step" key={step.id}>
      ...
      <button className="icon-button" disabled={index === 0} onClick={() => move(index, -1)} title="Move up" aria-label={`Move ${step.name} up`}><ArrowUp size={13} /></button><button className="icon-button" disabled={index === steps.length - 1} onClick={() => move(index, 1)} title="Move down" aria-label={`Move ${step.name} down`}><ArrowDown size={13} /></button><button className="icon-button danger-text" onClick={() => setSteps(steps.filter((_, position) => position !== index))} title="Remove" aria-label={`Remove ${step.name}`}><Trash2 size={13} /></button>
