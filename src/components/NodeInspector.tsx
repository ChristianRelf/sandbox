...
export function NodeInspector({ workflow, node, onChange, onDelete }: {
  ...
  return <aside className="inspector">
    ...
    <div className="inspector-scroll">
      ...
      {node.plugin && <><div className="info-note">Pinned to {node.plugin.pluginId} v{node.plugin.pluginVersion} ({node.plugin.packageIntegrity.slice(7,19)}). Updates never change this node automatically.</div><JsonField label="Plugin configuration" value={config} onChange={value=>onChange({...node,configuration:(value&&typeof value==="object"&&!Array.isArray(value)?value:{}) as Record<string,unknown>})}/><JsonField label="Input mapping" value={node.plugin.input??{}} onChange={value=>onChange({...node,plugin:{...node.plugin!,input:value}})}/></>}
