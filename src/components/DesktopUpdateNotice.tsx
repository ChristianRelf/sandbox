...
export function DesktopUpdateNotice({ collapsed = false }: { collapsed?: boolean }) {
  ...
  if (collapsed) return <button className="desktop-update-collapsed" title={`Sandbox ${update.version} is available`} aria-label={`Open Sandbox ${update.version} release`} onClick={() => void openRelease()}><Download size={15}/></button>;
  return <aside className="desktop-update-notice" aria-live="polite">
    <button className="desktop-update-link" title={`Open Sandbox ${update.version} release`} onClick={() => void openRelease()}><Download size={14}/><span>Update available</span></button>
    <button className="desktop-update-dismiss" aria-label={`Dismiss Sandbox ${update.version} update`} title="Dismiss" onClick={dismiss}><X size={13}/></button>
  </aside>;
}
