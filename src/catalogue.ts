...
const BASE_NODE_DEFINITIONS:NodeDefinitionInput[] = [
  ...
  {type:"code",name:"Code",description:"Author Python, HTML, JavaScript, or CSS in a full editor",group:"Data",icon:Code2,defaults:{language:"javascript",sourceCode:"// Transform data or provide browser-side JavaScript\n",executionMode:"source",timeoutMs:30000},summary:c=>`${String(c.language??"javascript").toUpperCase()} · ${c.executionMode==="run"?"execute":"source block"}`},
...
const NODE_PORTS:Partial<Record<BuiltInNodeType,{inputs:NodePortDefinition[];outputs:NodePortDefinition[]}>>={
  ...
  code:{inputs:[{key:"input",label:"Input",type:"any"}],outputs:[{key:"code",label:"Source code",type:"string"},{key:"result",label:"Result",type:"any"},{key:"stdout",label:"Standard output",type:"string"}]},
...
const SIDE_EFFECTS=new Set<NodeType>(["desktop_notification","move_file","write_file","copy_path","delete_path","run_command","code","web_builder","gmail_create_draft","gmail_send_email","gmail_add_label","discord_webhook","discord_embed","slack_webhook","approval","set_workflow_state","compare_previous"]);
