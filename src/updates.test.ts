...
describe("desktop update version comparison", () => {
  it("orders beta increments and stable releases correctly", () => {
    expect(isNewerVersion("0.7.2-beta.1", "0.7.2-beta.2")).toBe(true);
    expect(isNewerVersion("0.7.2-beta.2", "0.7.2")).toBe(true);
    expect(isNewerVersion("0.7.2", "0.7.2-beta.2")).toBe(false);
  });
