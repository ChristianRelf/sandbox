...
describe("desktop update version comparison", () => {
  it("orders beta increments and stable releases correctly", () => {
    expect(isNewerVersion("0.7.1-beta.0", "0.7.1-beta.3")).toBe(true);
    expect(isNewerVersion("0.7.1-beta.3", "0.7.1")).toBe(true);
    expect(isNewerVersion("0.7.1", "0.7.1-beta.3")).toBe(false);
  });
