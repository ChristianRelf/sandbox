...
| GA-016 | High | Support | v0.5.0 | Closed | Support access requires a dedicated platform role, a different fresh human workspace administrator's approval, one bounded diagnostics scope, a maximum eight-hour expiry and per-use reauthorization. Customers can reject or revoke immediately; diagnostics are aggregate and recursively redacted; all grant activity is sequenced and append-only. |
| GA-017 | High | Privacy | v0.5.0 | Closed | Typed workspace retention limits drive a bounded recurring purge/redaction worker for execution detail, queues, webhook payloads, runner commands and audit evidence. Account export is versioned and excludes secrets; deletion removes active access, pseudonymizes identity and cryptographically erases personal workflow content while retaining a minimal receipt. The published classification map distinguishes customer, secret, security and financial records. |
...
| GA-C10 | 2026-08-28 | API and PostgreSQL gates cover customer-approved support grants, self-approval denial, expiry/revocation enforcement, aggregate diagnostics redaction, and append-only access evidence. |
| GA-C11 | 2026-08-28 | Privacy API and PostgreSQL gates cover policy bounds, append-only retention-worker exception, classified account export, access removal, identity pseudonymization, personal-content cryptographic erasure and minimal deletion receipts. |
