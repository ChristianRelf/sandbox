...
| GA-012 | High | Identity | v0.5.0 | In progress | Workspace and organisation service principals support explicit multi-workspace role/environment assignments, scoped expiring HMAC-hashed tokens, immediate revocation, human creation/ownership, RLS and non-interactive login enforcement. Multi-workspace creation and token issue require authorization in every affected workspace. Durable seven-day and one-day owner expiry notifications are claimed safely across replicas and retried after delivery failures. Workspace-bound Ed25519 client assertions are audience/lifetime/scope constrained, replay protected, and exchange for 15-minute tokens whose key revocation is immediate. Add periodic access reviews before closure. |
