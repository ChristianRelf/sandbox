...
| GA-013 | High | API | v0.5.0 | In progress | Compatibility/deprecation and rate-limit policies, structured transport errors, correlation IDs, encrypted 24-hour PostgreSQL idempotency replay, live OpenAPI with promoted health/marketplace/credential/service-account schemas, checked-in drift gate and typed SDK compatibility tests are implemented. Promote the remaining `JsonValue` request/response schemas before closure. |
