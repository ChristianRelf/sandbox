...
Complete this section on your Windows workstation in the repository checkout. Do not add these values to `/opt/sandbox/.env` on the existing website/account Droplet: that Compose stack does not run the control plane and does not read its database or Stripe settings. If a control-plane secret was added there, copy it to the protected file below and then remove the unused line from the Droplet.
