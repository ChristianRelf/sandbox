...
| Code editor **Code with AI** | You want to discuss and revise a Python, HTML, JavaScript, or CSS file | A right-side coding chat that updates the checked editor draft before you save |
...
The builder returns a replacement graph, not an execution. It can create built-in nodes, control edges, typed input bindings, and complete Code node source blocks. It understands the capabilities and outputs of the built-in node catalogue, so a request such as an uptime monitor uses HTTP, state, condition, and notification nodes for the monitoring work while Code and Web Builder nodes create the interface. Web Builder's three typed inputs are repaired from the matching Code-node languages when a model returns a generic connection.

While a draft is in progress, the chat shows muted activity stages such as **Planning Draft**, **Investigating current workflow**, and **Writing Code**. These are concise operation indicators rather than hidden model reasoning.
...
Open a Code node, choose **Edit code**, then choose **Code with AI**. A coding chat opens to the right of the editor. Select a connection and describe the complete file or requested change. The current language and unsaved editor draft are included as context on every turn.

Each completed response replaces only the unsaved draft and remains in the chat so you can refine it. The Problems panel immediately checks the returned source. Choose **Save code** or press `Ctrl+S` / `Cmd+S` to commit it to the node; cancel the dialog to keep the previously saved source.
