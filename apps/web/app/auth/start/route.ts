...
export async function GET(request:NextRequest) {
  const authorize=required("OIDC_AUTHORIZE_URL"),clientId=required("OIDC_CLIENT_ID"),redirectUri=required("OIDC_REDIRECT_URI"),audience=required("OIDC_AUDIENCE");
  const state=randomBytes(32).toString("base64url"),verifier=randomBytes(48).toString("base64url");
  const challenge=createHash("sha256").update(verifier).digest("base64url");
  const target=new URL(authorize);
  target.search=new URLSearchParams({client_id:clientId,redirect_uri:redirectUri,response_type:"code",scope:"openid email profile",audience,state,code_challenge:challenge,code_challenge_method:"S256"}).toString();
