...
export function ProductPageView({ page }: { page: ProductPage }) {
  return <main id="content" className="detail-page">
    ...
    <section className="related-section"><div><h2>Continue in the documentation</h2><p>Detailed setup, security notes and troubleshooting live in the technical manual.</p></div><div>{page.related.map(path=><a key={path} href={`https://docs.sndbox.app/${path}`}>{path.split("/").at(-1)?.replaceAll("-"," ")} <ArrowRight size={13}/></a>)}</div></section>
