...
Sandbox is a local-first visual desktop automation platform. The v0.7 beta aligns the Windows desktop app, workflow engine, plugin runtime, public web surfaces, and Linux runners on one prerelease version.

## Beta distribution

- Windows testers install the Authenticode-signed `.exe` or `.msi` from the GitHub release.
- Linux x64 and ARM64 servers use the signed headless runner archive or the multi-architecture `sandbox-server-runner` container.
...
- A small DigitalOcean deployment is provided in `deploy/digitalocean`; see its README for the 1 GB Droplet path.
...
- Publication still requires the protected GitHub release environment, a valid Windows signing certificate, and an immutable `v0.7.0-beta.1` tag.
